...
namespace ServiceSystem.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [Route("GetUserRoles")]
        public IHttpActionResult GetUserRoles([FromUri]string email)
        {
            try
            {
                ...
                {
                    string cmdString = @"SELECT B.Name as RoleName
                                         FROM AspNetUsers A
                                         INNER JOIN AspNetUserRoles C ON A.Id =                       C.UserId
                                         INNER JOIN AspNetRoles B ON B.Id =                           C.RoleId
                                         WHERE A.Email = @email";
                    ...
                    using (SqlCommand cmd = new SqlCommand(cmdString, con))
                    {
                        ...
                        using (SqlDataReader rdr = cmd.ExecuteReader())
                        {
                            while (rdr.Read())
                            {
                                toReturn.Add(rdr["RoleName"].ToString());
                            }
                        }
                    }
                }
