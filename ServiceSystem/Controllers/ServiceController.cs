        ...
        public ActionResult Main(FormCollection collection, IEnumerable<HttpPostedFileBase> service_attachments)
        {
            try
            {
                ...
                using (HttpClient client = WebApiClient.InitializeAuthorizedClient(Request.Url.Scheme + "://" + Request.Url.Authority + Request.ApplicationPath.TrimEnd('/') + "/", "Bearer", Request.Cookies["access_token"].Value))
                {
                    ...
                    if (message.IsSuccessStatusCode)
                    {
                        return RedirectToAction("Main", "Service", new { message="Success|Послугу успішно створено. Перегляньте її сторінку з Особистого кабінету" });
                    }
                    else
                    {
                        return RedirectToAction("Main", "Service", new { message="Error|Відбулася помилка. Будь ласка, повторіть спробу" });
                    }
                }
            }
            catch
